...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const priceInputs = {
        COU_PRIX_REPAS: { el: document.querySelector('[name="COU_PRIX_REPAS"]'), maxDigits: 8, label: 'Prix du repas' },
        COU_REDUC_LICENCIE: { el: document.querySelector('[name="COU_REDUC_LICENCIE"]'), maxDigits: 8, label: 'Réduction licencié' },
    };
    ...
    function enforceMaxDigits(input, maxDigits, mode) {
        ...
        if (length <= maxDigits) {
            if (mode === 'price' && priceFractionDigitsLength(input.value) > 2) {
    ...
    function validateMaxDigits() {
        ...
        for (const key in priceInputs) {
            ...
            if (priceFractionDigitsLength(c.el.value) > 2) {
                errors.push(`${c.label} est limité à 8 chiffres avant la virgule et 2 après.`);
            }
        }
