    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-10">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-bold text-black">Raids à venir</h2>
                <span class="bg-[#7DC2A5] text-black py-1 px-3 rounded-full text-xs font-bold">
                    {{ count($upcomingRaids) }} raid(s)
                </span>
            </div>

            @forelse($upcomingRaids as $raid)
                <div class="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden mb-4">
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <button 
                            ...
                            class="rounded-xl bg-black text-white px-4 py-2 text-sm font-semibold hover:bg-gray-800 transition-colors"
                        >
                            Détails
                        </button>
                    </div>
            ...
            @empty
                <div class="rounded-2xl border border-black/5 bg-white/60 p-10 text-center shadow-sm">
                    ...
                    <p class="text-slate-600">Aucun raid à venir pour le moment.</p>
                </div>
            @endforelse
        </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-bold text-black">Raids passés</h2>
                <span class="bg-gray-200 text-black/70 py-1 px-3 rounded-full text-xs font-bold">
                    {{ count($pastRaids) }} raid(s)
                </span>
            </div>

            @forelse($pastRaids as $raid)
                <div class="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden mb-4 opacity-75">
                    <div class="px-6 py-4 flex items-center justify-between">
                        <div class="flex-1">
                            <h3 class="font-bold text-lg text-black">{{ $raid->RAID_NOM }}</h3>
                            <div class="flex items-center gap-4 mt-2 text-sm text-slate-600">
                        ...
                        <button 
                            type="button"
                            @click="openRaid = {{ $raid->RAID_NUM }}"
                            class="rounded-xl bg-gray-600 text-white px-4 py-2 text-sm font-semibold hover:bg-gray-700 transition-colors"
                        >
                            Détails
                        </button>
                    </div>

                    {{-- Modal de détails (identique aux raids à venir) --}}
                    <div
                        x-cloak
                        x-show="openRaid == {{ $raid->RAID_NUM }}"
                        x-transition.opacity
                    ...
                    >
                        ...
                        <div class="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-xl border p-6">
                            ...
                            <div class="mb-6">
                                <h4 class="font-bold text-black mb-4">Courses du raid</h4>
                                @if($raid->courses && $raid->courses->count() > 0)
                                    <div class="space-y-2">
                                        @foreach($raid->courses as $course)
                                ...
                                @else
                                    <div class="text-center py-8 text-slate-500 bg-gray-50 rounded-lg">
                                        <p class="text-sm">Aucune course pour ce raid.</p>
                                    </div>
                                @endif
                            </div>
            ...
            @empty
                <div class="rounded-2xl border border-black/5 bg-white/60 p-10 text-center shadow-sm">
                    ...
                    <p class="text-slate-600">Aucun raid passé.</p>
                </div>
            @endforelse
        </div>
    </div>
