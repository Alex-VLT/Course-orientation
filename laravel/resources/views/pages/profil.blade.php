                ...
                <div>
                    ...
                    <div class="mt-4 space-y-1 text-sm text-slate-700">
                        ...
                        <div>
                            ...
                            @if(!empty($user->CLU_NOM))
                                {{ $user->CLU_NOM }} ({{ $user->CLU_VILLE }})
                            @else
                                —
                            @endif
                        </div>
                    ...
                    >
                        ...
                        <div class="relative w-full max-w-xl rounded-2xl bg-white shadow-xl border p-6">
                            ...
                            <div class="mt-6 flex items-center justify-end gap-3">
                                <a href="{{ route('race.show', $c->COU_NUM) }}" class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">Détails de la course</a>
                            </div>
                        </div>
                    </div>
                @empty
                ...
                @forelse ($coursesPassees as $c)
                    ...
                    <div class="py-4 flex items-start justify-between gap-4 opacity-60">
                        <div>
                            <div class="font-semibold text-slate-900">
                                {{ $c->COU_NOM }}
                                <span class="ml-2 text-sm font-normal text-slate-500">— {{ $c->RAID_NOM }}</span>
                            </div>
                            <div class="text-sm text-slate-600 mt-1">
                                {{ \Carbon\Carbon::parse($c->COU_DATE_DEPART)->format('d/m/Y H:i') }}
                                → {{ \Carbon\Carbon::parse($c->COU_DATE_FIN)->format('d/m/Y H:i') }}
                        ...
                        <div class="shrink-0 flex flex-col items-end gap-2">
                            <button type="button" @click="openPast = {{ $c->COU_NUM }}" class="rounded-xl border px-3 py-2 text-xs font-semibold hover:bg-slate-50">Résultats</button>
                            <a href="{{ route('race.show', $c->COU_NUM) }}" class="rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-800">Détails</a>
                            <span class="rounded-full bg-slate-100 text-slate-700 border border-slate-200 px-3 py-1 text-xs font-semibold">Terminée</span>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div class="relative w-full max-w-xl rounded-2xl bg-white shadow-xl border p-6 opacity-100">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <button type="button" @click="openPast = null" class="rounded-xl px-3 py-2 text-sm font-semibold hover:bg-slate-100">Fermer</button>
                            </div>
                            ...
                            <div class="mt-6 flex items-center justify-end gap-3">
                                <button type="button" @click="openPast = null" class="rounded-xl border px-4 py-2 text-sm font-semibold hover:bg-slate-50">Fermer</button>
                                <a href="{{ route('race.show', $c->COU_NUM) }}" class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">Détails de la course</a>
                            </div>
                        </div>
                    </div>
    ...
    >
        ...
        <div class="relative w-full max-w-2xl rounded-2xl bg-white shadow-xl border p-6 overflow-y-auto max-h-[90vh]">
            <div class="flex items-start justify-between gap-4">
                ...
                <button type="button" @click="openEdit = false" class="rounded-xl px-3 py-2 text-sm font-semibold hover:bg-slate-100">Fermer</button>
            </div>
            ...
            <form 
                ...
                x-data="{
                    selectedClub: '{{ old('club_id', $user->CLU_NUM ?? '') }}',
                    licenceNumber: '{{ old('INS_NUM_LICENCE', $user->INS_NUM_LICENCE ?? '') }}'
                }"
            >
                @csrf
                @method('PUT')
                            ...
                            >
                                ...
                                @if(isset($clubs))
                                    @foreach($clubs as $club)
                                        <option value="{{ $club->CLU_NUM }}">
                                            {{ $club->CLU_NOM }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                            @error('club_id', 'profileUpdate') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>

                <div class="flex items-center justify-end gap-3 pt-2">
                    <button type="button" @click="openEdit = false" class="rounded-xl border px-4 py-2 text-sm font-semibold hover:bg-slate-50">Annuler</button>
