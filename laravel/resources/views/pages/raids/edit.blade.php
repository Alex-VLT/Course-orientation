...
@section('content')
    <div class="max-w-3xl mx-auto py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">Modifier le raid</h1>
            <a href="{{ route('raids.manager') }}" class="text-sm text-blue-600 hover:underline">← Retour à la gestion</a>
        </div>
        ...
        <form action="{{ route('raids.update', $raid->RAID_NUM) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4 bg-white p-6 rounded-md shadow-sm">
                <div>
                    ...
                    <input name="RAID_NOM" value="{{ old('RAID_NOM', $raid->RAID_NOM) }}" class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" />
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input name="RAID_DATE_DEBUT" id="RAID_DATE_DEBUT" type="date" value="{{ old('RAID_DATE_DEBUT', optional($raid->RAID_DATE_DEBUT)->format('Y-m-d')) }}" min="{{ \Carbon\Carbon::today()->format('Y-m-d') }}" required class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" />
                    ...
                    <div>
                        ...
                        <input name="RAID_DATE_FIN" id="RAID_DATE_FIN" type="date" value="{{ old('RAID_DATE_FIN', optional($raid->RAID_DATE_FIN)->format('Y-m-d')) }}" class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" required />
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input name="RAID_DATE_DEBUT_INSCRI" id="RAID_DATE_DEBUT_INSCRI" type="date" value="{{ old('RAID_DATE_DEBUT_INSCRI', optional($raid->RAID_DATE_DEBUT_INSCRI)->format('Y-m-d')) }}" min="{{ \Carbon\Carbon::today()->format('Y-m-d') }}" required class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" />
                    ...
                    <div>
                        ...
                        <input name="RAID_DATE_FIN_INSCRI" id="RAID_DATE_FIN_INSCRI" type="date" value="{{ old('RAID_DATE_FIN_INSCRI', optional($raid->RAID_DATE_FIN_INSCRI)->format('Y-m-d')) }}" class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" required />
                ...
                <div>
                    ...
                    <select name="INS_ID" id="INS_ID" class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2">
                        @foreach ($members as $m)
                            <option value="{{ $m->INS_ID }}" data-club="{{ $m->CLU_NUM }}" {{ old('INS_ID', $raid->INS_ID) == $m->INS_ID ? 'selected' : '' }}>{{ $m->INS_PRENOM }} {{ $m->INS_NOM }} @if($m->INS_NUM_LICENCE) (Licence: {{ $m->INS_NUM_LICENCE }}) @endif</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input name="RAID_LIEN_SITE_WEB" value="{{ old('RAID_LIEN_SITE_WEB', $raid->RAID_LIEN_SITE_WEB) }}" placeholder="https://..." class="mt-1 w-full rounded-md bg-gray-100 px-3 py-2" />
                ...
                <div>
                    ...
                    <input type="text" 
                        name="RAID_CONTACT" 
                        id="RAID_CONTACT" 
                        value="{{ old('RAID_CONTACT', $raid->RAID_CONTACT) }}" 
                ...
                <div>
                    ...
                    @if($raid->RAID_ILLUSTRATION)
                        <div class="mt-2 mb-3">
                            <img src="{{ asset('images/' . $raid->RAID_ILLUSTRATION) }}" alt="Illustration actuelle" class="max-h-40 rounded border">
                            <p class="text-xs text-gray-600 mt-1">Illustration actuelle : {{ $raid->RAID_ILLUSTRATION }}</p>
                        </div>
                    @endif
                    <div class="mt-1 flex items-center gap-3">
                        ...
                        <span id="illustration-filename" class="text-sm text-gray-600"></span>
                    </div>
                    <p class="text-xs text-gray-600 mt-1">Laissez vide pour conserver l'illustration actuelle.</p>
                ...
                <div>
                    ...
                    <div id="map" data-lat="{{ old('RAID_LATITUDE', $raid->RAID_LATITUDE) }}" data-lng="{{ old('RAID_LONGITUDE', $raid->RAID_LONGITUDE) }}" class="mt-2 rounded-md overflow-hidden" style="height:300px"></div>
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-3">
                        ...
                        <div class="text-sm">Latitude: <span id="lat-display">{{ old('RAID_LATITUDE', $raid->RAID_LATITUDE) }}</span></div>
                        <div class="text-sm">Longitude: <span id="lng-display">{{ old('RAID_LONGITUDE', $raid->RAID_LONGITUDE) }}</span></div>
                    </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button class="rounded-md bg-black px-4 py-2 text-white hover:bg-gray-800">Enregistrer les modifications</button>
                </div>
